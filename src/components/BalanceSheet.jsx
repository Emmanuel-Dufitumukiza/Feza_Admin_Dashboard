...
const [end, setEnd]  = useState(null)
  ...
  const [currentUser, setCurrentUser] = useState({})
    ...
    const FIXEDLONGTERMASSETS =[];
    const LongTermInvestments =[]
    const PropertyPlantandEquipment =[]
    const LessAccumulatedDepreciation =[];
...
const TOTALLIABILITIESANDOWNEREQUITY = [];
const TOTALLIABLITIES = []
...
for(let j=0; j<periods.length; j++){
  ...
  if(startYear && endYear && startMon && endMon && startingTime && endingTime){
    ...
    getInventory(numberedPeriods,userId);
  }
...
const [prices44, setPrices44] = useState([])

const getInventory  =async(numberedPeriods,userId)=>{
  let prices = await axiosInstance.post("/financial/getInventory", {user: userId,dates: numberedPeriods})
  //  console.log(prices.data)
    // setPrices3(prices.data)
    setPrices44(prices.data)
}
...
let sum8 = 0;
  ...
  for (let j = 0; j <prices8[1]?.length; j ++) {
    ...
    for (let j = 0; j <prices_4[0]?.length; j ++) {
      ...
      PrepaidExpenses?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      ShortTermInvestments?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      LongTermInvestments?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      LessAccumulatedDepreciation?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      IntangibleAssets?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      DeferredIncomeTax?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      Other?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      ShortTermLoans?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      IncomeTaxesPayable?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      UnearnedRevenue?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      CurrentPortionofLongTermDebt?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      LongTermLoans?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      DeferredIncomeTax2?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
       Other2?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
       Other3?.push("RF "+0?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
    }
    ...
    for(let i=0; i<prices44?.length; i++){
        Inventory.push("RF " + prices44[i]?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
      }

      const prices_33 =[tableHead12,CashAtTheEndofPeriod,PrepaidExpenses,Inventory, ShortTermInvestments]
      let sum13 = 0;
      for (let j = 0; j <prices_33[0]?.length; j ++) {
        sum13 = 0;
        for(let a=0; a<prices_33.length; a++){
          let arr = (prices_33[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            sum13+= parseInt(arr[1]);
          }
        }
        TOTALCURRENTASSETS.push("RF " + sum13?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }

      const prices_44 =[LongTermInvestments, LessAccumulatedDepreciation,CashPaidfor,IntangibleAssets]
      let sum43 = 0;
      for (let j = 0; j <prices_44[0]?.length; j ++) {
        sum43 = 0;
        for(let a=0; a<prices_44.length; a++){
          let arr = (prices_44[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            sum43+= parseInt(arr[1]);
          }
        }
        TOTALFIXEDASSETS.push("RF " + sum43?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }  

      const prices_55 =[DeferredIncomeTax, Other]
      let sum45 = 0;
      for (let j = 0; j <prices_55[0]?.length; j ++) {
        sum45 = 0;
        for(let a=0; a<prices_55.length; a++){
          let arr = (prices_55[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            sum45+= parseInt(arr[1]);
          }
        }
        TOTALOTHERASSETS.push("RF " + sum45?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }
      
      const prices_66 =[TOTALFIXEDASSETS, TOTALCURRENTASSETS,TOTALOTHERASSETS]

      let sum65 = 0;
      for (let j = 0; j <prices_66[0]?.length; j ++) {
        sum65 = 0;
        for(let a=0; a<prices_66.length; a++){
          let arr = (prices_66[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            sum65+= parseInt(arr[1]);
          }
        }
        TOTALASSETS.push("RF " + sum65?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }

      const AccountsPayable= [];
      const prices_22 =[tableHead7,tableHead6,tableHead8,tableHead9]

      let sum11 = 0;
      for (let j = 0; j <prices_22[0]?.length; j ++) {
        sum11 = 0;
        for(let a=0; a<prices_22.length; a++){
          let arr = (prices_22[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
        sum11+= parseInt(arr[1]);
          }
        }
        AccountsPayable.push("RF " + sum11?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }

      const prices_77 =[ShortTermLoans,IncomeTaxesPayable,UnearnedRevenue,CurrentPortionofLongTermDebt,AccountsPayable,tableHead7]

      let sum67 = 0;
      for (let j = 0; j <prices_77[0]?.length; j ++) {
        sum67 = 0;
        for(let a=0; a<prices_77.length; a++){
          let arr = (prices_77[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            sum67+= parseInt(arr[1]);
          }
        }
        TOTALCURRENTLIABILITIES.push("RF " + sum67?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }

      const prices_88 =[Other2,DeferredIncomeTax2,LongTermLoans]

      let sum77 = 0;
      for (let j = 0; j <prices_88[0]?.length; j ++) {
        sum77 = 0;
        for(let a=0; a<prices_88.length; a++){
          let arr = (prices_88[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            sum77+= parseInt(arr[1]);
          }
        }
        TOTALLONGTERMLIABILITIES.push("RF " + sum77?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }

      const revenueLiablities = []

      const prices_99 =[TOTALLONGTERMLIABILITIES, TOTALCURRENTLIABILITIES]

      let sum84 = 0;
      for (let j = 0; j <prices_99[0]?.length; j ++) {
        sum84 = 0;
        for(let a=0; a<prices_99.length; a++){
          let arr = (prices_99[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            sum84+= parseInt(arr[1]);
          }
        }
        TOTALLIABLITIES.push("RF " + sum84?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }

      const ownersEquity = [TOTALLIABLITIES,NetIncome];

      let sum = 0;
      for (let j = 1; j <ownersEquity[0]?.length; j ++) {
        sum = 0;
        for(let a=0; a<ownersEquity.length; a++){
          let arr = (ownersEquity[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            sum+= parseInt(arr[1]);
          }
        }
        revenueLiablities.push("RF " + sum?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }
      
      const ownersEquityPrices = [TOTALASSETS,revenueLiablities];

      let total = 0;
      for (let j = 0; j <ownersEquityPrices[0]?.length; j ++) {
        total = 0;
        // for(let a=0; a<2; a++){
          let arr = (ownersEquityPrices[0][j])?.split(" ");
          let arr2 = (ownersEquityPrices[1][j])?.split(" ");
    
          if(arr?.length>0 && arr2?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            arr2[1] = arr2[1]?.split(",")?.join("")
    
            total = parseInt(arr[1]) -  parseInt(arr2[1]);
          }
          // }
          OwnerEquity.push("RF " + total?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }

      const totalownersEquity = [OwnerEquity,NetIncome,Other3];

      let sumt = 0;
      for (let j = 0; j <totalownersEquity[0]?.length; j ++) {
        sumt = 0;
        for(let a=0; a<totalownersEquity.length; a++){
          let arr = (totalownersEquity[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            sumt+= parseInt(arr[1]);
          }
        }
        TOTALOWNEREQUITY.push("RF " + sumt?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }

      const totalownersEquityLiab = [TOTALLIABLITIES, TOTALOWNEREQUITY];

      let sumtotal = 0;
      for (let j = 0; j <totalownersEquityLiab[0]?.length; j ++) {
        sumtotal = 0;
        for(let a=0; a<totalownersEquityLiab.length; a++){
          let arr = (totalownersEquityLiab[a][j])?.split(" ");
          if(arr?.length>0){
            arr[1] = arr[1]?.split(",")?.join("")
            sumtotal+= parseInt(arr[1]);
          }
        }
        TOTALLIABILITIESANDOWNEREQUITY.push("RF " + sumtotal?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      }
   ...
   return(
      ...
      <div className="smb-page darkmode darkmode2 px-3 pt-3 pb-3">
        ...
        <button style={{position: "relative", top: "3px"}} onClick={()=>navigate("/home")} className='btn btn-primary shadow-none backBtn float-start ms-4 me-5'>Back</button>
          ...
          <h3 className='text-white text-center mb-4'>Balance Sheet</h3>
...
<p className="text-muted lit-text pb-1 float-end">Admin</p>
    ...
    <div className="profile ms-3">
        ...
        <Table3 TOTALLIABILITIESANDOWNEREQUITY={TOTALLIABILITIESANDOWNEREQUITY} OwnerEquity={OwnerEquity} TOTALOWNEREQUITY={TOTALOWNEREQUITY} Other3={Other3} TOTALLIABLITIES={TOTALLIABLITIES} TOTALLONGTERMLIABILITIES={TOTALLONGTERMLIABILITIES} Other2={Other2} DeferredIncomeTax2={DeferredIncomeTax2} LongTermLoans={LongTermLoans}		 TOTALCURRENTLIABILITIES={TOTALCURRENTLIABILITIES} UnearnedRevenue={UnearnedRevenue} CurrentPortionofLongTermDebt={CurrentPortionofLongTermDebt} ShortTermLoans={ShortTermLoans} IncomeTaxesPayable={IncomeTaxesPayable} AccountsPayable={AccountsPayable} TOTALASSETS={TOTALASSETS} DeferredIncomeTax={DeferredIncomeTax} Other={Other} TOTALOTHERASSETS={TOTALOTHERASSETS} TOTALFIXEDASSETS={TOTALFIXEDASSETS} LessAccumulatedDepreciation={LessAccumulatedDepreciation} IntangibleAssets={IntangibleAssets} CashPaidfor={CashPaidfor} LongTermInvestments={LongTermInvestments} TOTALCURRENTASSETS={TOTALCURRENTASSETS} ShortTermInvestments={ShortTermInvestments} PrepaidExpenses={PrepaidExpenses} Inventory={Inventory} tableHead12={tableHead12} CashAtTheEndofPeriod={CashAtTheEndofPeriod} CashAtTheBeginningofPeriod={CashAtTheBeginningofPeriod} NetIncreaseinCash={NetIncreaseinCash} NetCashFlowfromFinancingActivites={NetCashFlowfromFinancingActivites} CashPaidfor2={CashPaidfor2} CashReceiptsfrom2={CashReceiptsfrom2} LoanRepayment2={LoanRepayment2} Dividends={Dividends} Borrowed={Borrowed} MyPersonalMoneyIaddedtobusiness={MyPersonalMoneyIaddedtobusiness} NetCashFlowfromInvestingActivities={NetCashFlowfromInvestingActivities} CashReceiptsfrom={CashReceiptsfrom}  PurchaseofEquipment={PurchaseofEquipment} LoanRepayment={LoanRepayment} PurchaseofProperty={PurchaseofProperty} PurchaseofBusiness={PurchaseofBusiness} opC={opC} CashLost={CashLost} cashGotten={cashGotten} tableHead7={tableHead7} tableHead6={tableHead6} tableHead8={tableHead8} tableHead9={tableHead9} prices8={prices8} prices7={prices7} opMargins={opMargins} NetProfitMargin={NetProfitMargin} grossMargins={grossMargins} NetIncome={NetIncome} totalNonOpEx={totalNonOpEx} opProfit={opProfit} totalOpEx={totalOpEx} ranges={ran} grossProfits={grossProfits} totalcogs={totalcogs} totalRevenues ={totalRevenues} selectedNonExpenses={selectedNonExpenses} prices5={prices5} prices={prices} prices4={prices4} prices2={prices2} selectedExpenses={selectedExpenses} selectedSales={selectedSales}/>
        <br/>
        </div>
        </div>
